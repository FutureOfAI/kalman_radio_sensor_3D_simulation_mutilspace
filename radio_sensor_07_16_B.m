% Program name: radio_sensor_07_16_B.m
...
% 7/16/2017
% 7/19/2017
% This version is assuming the platform's gyro biases are already
% calibrated using the program: radio_sensor_07_16_A
% In this case, we will used the 3-axis accelerometers and 3-axis
% magnetometers and TRIAD algorithm to correct the attitude errors through
% the use of 3-states Kalmsn filter
close all;
clear all;
clc;
format short e
...
%================================
  ...
  %      T = 1;
        ...
        delta_s = 1*delta_t;                           % sampling at every 0.5 second for the Kalman filter
        err_flag = 1;                                   % flag for turning on the magnetometer and accelerometer errors
        err_flag_g = 0;                                 % flag for turning in the gyro noises
  % ================================================
  % ================================================
        t0 = 0:dt:T;
        t0_1 = t0';
...
% ==============================================================
%bgz0*r2d,
[s3_xz_h,s3_P00_z]=define_initial_condition_3(phierr,thetaerr,psierr,d2r);
% ==============================================================
% for 6-states filter
s3_F_z=zeros(3);
...
s3_R = 1*[(0.5*d2r)^2 0 0                     % TRIAD attitude determination error in x - axis
...
s3_Q_z=zeros(3);
s3_Q_z(1:3,1:3) = [sig_x_arw^2 0 0 
                    0 sig_y_arw^2 0 
                    0 0 sig_z_arw^2]; 
% ============================================================
% ============================================================
...
%    dQ1 = qmult(inv_QE_B_m,QE_B);
    ...
    dq31(k) = 2*dQ1(3,1);
% ===========================================================
% Perform Kalman filter propagation for 3-state Kalman filter
% ===========================================================
        s3_phi_z = expm(s3_F_z*dt);
        %
        %[s6_xz_h,s6_P00_z]=Kalman_Filter_estimate6_radio(s6_xz_h,s6_phi_z,s6_P00_z,s6_Q_z,dt);
        s3_xz_h=s3_phi_z*s3_xz_h;
        s3_P00_z=s3_phi_z*s3_P00_z*(s3_phi_z') + s3_Q_z*dt;
...
%     [s6_P00_z,s6_K_z,s6_z_update,s6_Mu_z]=Kalman_Filter_update_6_1_radio(s6_P00_z,axm,aym,azm,x_a_N,y_a_N,z_a_N,mag_xm,mag_ym,mag_zm,mag_E,DC_E_B_m,s6_H,s6_R,k);
    ...
    %Q_E_B_e,
    [s3_P00_z,s3_K_z,s3_z_update,s3_Mu_z,dQ,dQ11]=Kalman_Filter_update_7_8_B_radio(s3_P00_z,Q_E_B_e,QE_B_m,s3_H,s3_R,Q_E_B,k);
%    Q_E_B_e,
%    [time(k) k],
%    [dQ dQ11],
%
    [dtheda_xh,dtheda_yh,dtheda_zh]=upon_radiosensor_measurement_6_1_B(dtheda_xh,dtheda_yh,dtheda_zh,k,s3_z_update);
