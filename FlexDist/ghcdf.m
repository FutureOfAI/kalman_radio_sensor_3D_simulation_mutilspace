function p = ghcdf(x,Param,m,sigma)
%GHCDF Generalized Hyperbolic cumulative density function (cdf).
%   Y = GHCDF(X,PARAM,M,SIGMA) Returns the GH cdf
...
%   (1999).
%   This function uses an integral calculation.
%
...
p(Zeta <= 0 | abs(Rho) >= 1 | sigma <= 0 | isnan(x) | isnan(m) | ...
    isnan(sigma) | isnan(Lambda) | isnan(Zeta) | isnan(Rho)) = NaN;

% Since GHCDF uses a numerical integration to compute the c.d.f., specify a
% lower bound to avoid 'infinite number' error.
LowerBound = m - 100*sigma; % This should be -Inf.
p(Zeta > 0 & abs(Rho) < 1 & sigma > 0 & x <= LowerBound) = 0;

h   =   find(Zeta > 0 & abs(Rho) < 1 & sigma > 0 & x > LowerBound);

if any(h)
    fun =  @(z, lambda, zeta, rho, mu, sig, LB) integral(@(t) ghpdf(t,[lambda, zeta, rho],...
        mu,sig), LB, z);
    p(h) = arrayfun(fun, x(h), Lambda(h), Zeta(h), Rho(h), m(h), sigma(h), LowerBound(h));
end