...
%a_Nh = inv(DC_E_B_m(:,:,1,k-1))*[axm_h(k-1); aym_h(k-1); azm_h(k-1)]+[0; 0; -9.8];
a_Nh = inv(DC_E_B_m(:,:,k))*[axm_h(k); aym_h(k); azm_h(k)]+[0; 0; -9.8];
xam_Nh(k) = a_Nh(1);
yam_Nh(k) = a_Nh(2);
zam_Nh(k) = a_Nh(3);
%
%a_Nh = inv(DC_E_B_m(:,:,:,k-1))*[axm_h(k-1); aym_h(k-1); azm_h(k-1)]+[0; 0; -9.8];
%xam_Nh(k-1) = a_Nh(1);
%yam_Nh(k-1) = a_Nh(2);
%zam_Nh(k-1) = a_Nh(3);
%
